...
| Go language tooling | `go`; Mason's [gopls](https://raw.githubusercontent.com/mason-org/mason-registry/main/packages/gopls/package.yaml) and [delve](https://raw.githubusercontent.com/mason-org/mason-registry/main/packages/delve/package.yaml) entries declare Go package sources. | **Capability runtime:** manage Go with mise. Homebrew's [`go`](https://formulae.brew.sh/formula/go) formula is available on both platforms. |
| Rust language tooling and native plugin fallback | `cargo` and `clippy`; the tracked rust-analyzer settings load all Cargo features and use `clippy` for checks. The tracked FFF install hook calls `download_or_build_binary()`, whose upstream contract downloads a prebuilt binary or falls back to a Cargo build. ([FFF installation](https://github.com/dmtrKovalenko/fff#installation)) | **Capability runtime/build fallback:** manage Rust with mise. Mise delegates to rustup and its default profile includes Cargo and Clippy; if selecting the minimal profile, add the `clippy` component explicitly. ([mise Rust backend](https://mise.jdx.dev/lang/rust.html), [rustup components](https://rust-lang.github.io/rustup/concepts/components.html)) |
