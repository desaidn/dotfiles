...
Neovim-owned terminal flows should use the shared `nvim/lua/custom/lib/terminal_tool.lua` launcher: persistent fullscreen tmux popup inside tmux, floating terminal outside tmux, shell-owned editor variables, and flatten.nvim handoff. New tools should be thin declarations over that helper unless they have a genuinely different job.
