...
| Java                  | Generic `jdtls`; JDT formatting disabled; Google Java Format via Conform                                               | No Java debugger/test bundles or nvim-jdtls refactors. The upstream default JDT workspace path is based on only the root basename, so two worktrees/projects with the same directory name can collide.                                                                                   |
