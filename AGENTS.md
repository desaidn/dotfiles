...
- Each tool maintains its own subdirectory under the repo root, mirroring the XDG layout under `~/.config/`; Hunk links only `config.toml` so mutable runtime state stays untracked
